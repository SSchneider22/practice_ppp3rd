...
@application.route('/') # routeメソッドは、URLに対して実行する関数を登録するデコレータ
def index():
    """トップページ
    ...
    # 記録データを読み込みます
    rides = load_data()
    return render_template('index.html', rides=rides) # 指定したファイルをテンプレートとして使い、テンプレートエンジンによってHTMLを出力する関数。キーワード引数で指定した値を、テンプレート変数として使用できる
...
@application.template_filter('nl2br') # template_filterメソッドは、関数を指定した名前のテンプレートフィルターとして登録するためのデコレータ
def nl2br_filter(s):
    """改行文字をbrタグに置き換えるテンプレートフィルター、windows対応のため、CRLF（\r\n）も入れている"""
